...
event zeek_init()
{
    ...
    SumStats::create([$name = "detect http scan based on 404 resp",
                      $epoch = 10min,
                      $reducers = set(r1,r2,r3),
                      $epoch_result(ts: time, key: SumStats::Key, result: SumStats::Result) = 
                        {
                            local re1 : double = result["count of 404 resp"]$sum / result["count of resp"]$sum;
                            local re2 : double = result["count of uni 404 resp"]$unique / result["count of 404 resp"]$sum;
